...
LiquidCrystal_I2C lcd(0x27, 20, 2);               // Configuration ecran 2 ligne LCD;
...
long transverter = 225600000;     // Fréquence FI TX
long convertisseur = 1001745000;  // Fréquence FI RX

long FreqTrans;                   // Fréquence réélle sortie de transverter
long FreqConver;                  // Fréquence réélle sortie de convertisseur
...
void affichage(){
    FreqTrans = (transverter + memFreq817);
    FreqConver = (convertisseur + memFreq818);  
    lcd.setCursor(0, 0);
    lcd.print("TX ");
    lcd.setCursor(4,0);
    lcd.print(FreqTrans);
...
void setup() {                          // Déclaration des ports de l'Arduino
  ...
  pinMode(TX, OUTPUT);
  pinMode(TX2, OUTPUT);
  pinMode(interPin, INPUT);             // Entrée de l'inverseur
